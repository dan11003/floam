...
typedef struct
{
  ...
  bool fixed;
  std::unordered_map<unsigned int,Eigen::Vector3d> mean; //  these means the subset of all point which have a correspondance, idex provides mapping to original
  std::unordered_map<unsigned int,Eigen::Vector3d> normal;
}Frame;

//std::vector<Correspondance> correspondences

    //pairwise
    Isometry3d pointToPoint_EigenQuaternion(std::vector<Vector3d> &src, std::vector<Vector3d> &dst);
    Isometry3d pointToPoint_CeresAngleAxis(std::vector<Vector3d> &src, std::vector<Vector3d> &dst);

    Isometry3d pointToPlane_EigenQuaternion(std::vector<Vector3d> &src, std::vector<Vector3d> &dst, std::vector<Vector3d> &nor);
    Isometry3d pointToPlane_CeresAngleAxis(std::vector<Vector3d> &src, std::vector<Vector3d> &dst, std::vector<Vector3d> &nor);

    //multiview
    void ceresOptimizer(std::vector<std::shared_ptr<Frame> >& frames, const std::vector<Correspondance>& correspondences, bool pointToPlane, bool robust);
    void ceresOptimizer_ceresAngleAxis(std::vector< std::shared_ptr<Frame> >& frames, const std::vector<Correspondance>& correspondences,  bool pointToPlane, bool robust);
