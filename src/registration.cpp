...
Eigen::Isometry3d eigenQuaternionToIso(const Eigen::Quaterniond& q, const Vector3d& t){
    Isometry3d poseFinal = Isometry3d::Identity();
...
Eigen::Isometry3d pointToPoint_CeresAngleAxis(std::vector<Vector3d>&src, std::vector<Vector3d>&dst){
...
void ceresOptimizer(std::vector<std::shared_ptr<Frame> >& frames, const std::vector<Correspondance>& correspondences, bool pointToPlane, bool robust){
    ...
    for(int i=0; i<frames.size(); i++){
      ...
      qs[i] = Eigen::Quaterniond(originalPose.linear());
      ts[i] = Eigen::Vector3d(originalPose.translation());
